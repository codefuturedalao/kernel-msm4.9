...
static ssize_t mm_addr_write(struct file *file, const char __user *buf,
				size_t count, loff_t *offset)
{
	/* cannot write */
	return 0;
...
static int mm_addr_show(struct seq_file *m, void *v)
{
	struct inode *inode = m->private;
	struct task_struct *p;
...
static int mm_addr_open(struct inode *inode, struct file *filp)
{
	return single_open(filp, mm_addr_show, inode);
}

static const struct file_operations proc_pid_set_mm_addr_operations = {
	.open		= mm_addr_open,
	.read		= seq_read,
	.write		= mm_addr_write,
	.llseek		= seq_lseek,
...
static const struct pid_entry tgid_base_stuff[] = {
	...
	REG("mm_addr",      S_IRUGO|S_IWUSR, proc_pid_set_mm_addr_operations),
#ifdef CONFIG_HAVE_ARCH_TRACEHOOK
...
static int proc_tid_mm_addr_permission(struct inode *inode, int mask)
{
	bool is_same_tgroup;
	struct task_struct *task;
	...
	if (likely(is_same_tgroup && !(mask & MAY_EXEC))) {
		/* This file (/proc/<pid>/task/<tid>/mm_addr) can always be
		 * read or written by the members of the corresponding
		 * thread group.
		 */
		return 0;
...
static const struct inode_operations proc_tid_mm_addr_inode_operations = {
		.permission = proc_tid_mm_addr_permission,
};
/*
 * Tasks
 */
static const struct pid_entry tid_base_stuff[] = {
	...
	NOD("mm_addr",      S_IFREG|S_IRUGO|S_IWUSR,
			 &proc_tid_mm_addr_inode_operations,
			 &proc_pid_set_mm_addr_operations, {}),
#ifdef CONFIG_HAVE_ARCH_TRACEHOOK
	ONE("syscall",   S_IRUSR, proc_pid_syscall),
#endif
	REG("cmdline",   S_IRUGO, proc_pid_cmdline_ops),
